...
select MAX(purchase_amount),customer_id from Orders group by customer_id ASC;

select MAX(purchase_amount),customer_id from Orders group by customer_id ASC;

select MAX(purchase_amount),customer_id from Orders ASC group by customer_id;
...
select A.customer_name as CUST_NAME,A.city as CUST_CITY,B.salesman_id as SALESMAN_ID,B.commission as SALESMAN_COMM from customers a INNER JOIN salesman b ON a.salesman_id=b.salesman_id;
...
SELECT grade, COUNT(*) as "Customers with grade" FROM customers 
GROUP BY grade HAVING grade>(SELECT AVG(grade) FROM customers WHERE city='New York');
